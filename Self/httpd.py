...
class HTTPServer:
    def __init__(self,localhost,port):
        ...
        self.port = port
        self.socket_object = self.startserver()
        self.sendresponse()
    def startserver(self):
        s_object = socket.socket()
        s_object.bind((self.localhost,self.port))
        s_object.listen(5)
        return s_object
    def sendresponse(self):
        while True:
            connect, address = self.socket_object.accept()
            ...
            '''
            connect.sendall(http_response.encode())
            connect.close()
